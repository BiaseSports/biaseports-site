...
export default function Evaluation() {
  return (
    <main className="max-w-4xl mx-auto px-6 py-12">
      ...
      <h1 className="mt-3 text-3xl font-extrabold">Evaluation Session</h1>
      <div className="mt-2 text-neutral-600">$45</div>

      <p className="mt-5 text-neutral-700">
        Your starting point: technical, tactical & physical assessment, Sock Level baseline,
        and a personalized training roadmap.
      </p>

      <div className="mt-6">
        <h2 className="font-semibold text-lg">What’s included</h2>
        <ul className="mt-2 list-disc pl-5 space-y-1 text-neutral-700">
          <li>60-minute evaluation</li>
          <li>Sock Level baseline & goals</li>
  ...
  return (
    <section className="mt-10 rounded-2xl border border-neutral-200 p-5">
      ...
      <p className="mt-2 text-neutral-700">
        Pay via <strong>Zelle</strong> or <strong>Venmo</strong>. Include the player’s name in the memo.
      </p>
...
function PayBox({ title, value, href, external }) {
  return (
    <div className="rounded-xl border p-4">
      <h3 className="font-semibold">{title}</h3>
      <p className="mt-1 text-neutral-700">
        Send to:{" "}
        <a className="underline" href={href} {...(external ? { target: "_blank", rel: "noopener noreferrer" } : {})}>
          <strong>{value}</strong>
        </a>
      </p>
    </div>
